...
class Stat_Window(Mucked.Seat_Window):
    ...
    def create_contents(self, i):
        ...
        self.add(self.grid)
        log("Stat_Window rows: "+str(self.aw.rows)+", cols:"+str(self.aw.cols))
...
#    layout is handled by superclass!
        log("SimpleHUD rows: "+str(self.nrows)+", cols:"+str(self.ncols))
        self.stats = [None for i in range(self.nrows*self.cols) ]
        for stat in self.game.stats:
            log("Simple_HUD stat: "+str(self.config.supported_games[self.poker_game].stats[stat].stat_name))
